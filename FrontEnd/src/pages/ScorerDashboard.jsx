...
export default function ScorerDashboard() {
    ...
    try {
      ...
      if (ballData.isWicket) {
        ...
        if (updatedInnings.wickets + 1 >= 10) {
          setTimeout(() => {
            alert(
              '10 wickets down! Click "End Innings" to proceed to next innings.'
            );
          }, 500);
        }
      }
  ...
  const handleEndInnings = async () => {
    const currentInnings =
      liveMatchData.innings[liveMatchData.currentInnings - 1];
    const totalOvers = liveMatchData.totalOvers || 20; // Default to 20 overs if not provided

    // ✅ Auto-end innings if 10 wickets or 20 overs complete
    if (currentInnings.wickets >= 10 || currentInnings.overs >= totalOvers) {
      console.log(
        `✅ Innings ${liveMatchData.currentInnings} auto-complete triggered`
      );
    } else {
      // 🧠 Ask for confirmation if scorer ends early
    ...
    try {
      ...
      // Clear fields for next innings or match
      setCurrentBatsman("");
      setCurrentBowler("");
      setLastBalls([]);
