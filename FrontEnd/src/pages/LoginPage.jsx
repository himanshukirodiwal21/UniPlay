...
export default function AuthPages() {
  ...
  const handleAdminLoginChange = (e) => {
    const { name, value } = e.target;
    setAdminLoginData((p) => ({ ...p, [name]: value }));
    if (errors[name]) setErrors((p) => ({ ...p, [name]: "" }));
  };
  ...
  const validateSignup = () => {
    const e = {};
    if (!formData.fullName) e.fullName = "Full name required";
  ...
  const handleVerifyOTP = async () => {
    if (!formData.otp) return setErrors({ otp: "OTP required" });
    setIsSubmitting(true);

    try {
      await api.post("/api/v1/users/verifyemail", {
        code: formData.otp,
      });
      setStep(3);
    } catch (err) {
      setErrors({ otp: err.response?.data?.message || "OTP failed" });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", justifyContent: "center", alignItems: "center" }}>
      <div style={{ width: 380, padding: 32, borderRadius: 12, background: "#fff" }}>
        ...
        {currentPage === "login" && (
          <>
            ...
            <button onClick={handleLogin} disabled={isSubmitting}>Login</button>
            <button onClick={() => setCurrentPage("signup")}>Sign Up</button>
            <button onClick={() => setCurrentPage("adminLogin")}>Admin Login</button>
          </>
        )}
        ...
        {currentPage === "adminLogin" && (
          <>
            <h2>Admin Login</h2>
            <input name="email" placeholder="Email" onChange={handleAdminLoginChange} />
            <input type="password" name="password" placeholder="Password" onChange={handleAdminLoginChange} />
